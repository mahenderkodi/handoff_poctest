...
- [scripts/bootstrap-handoff.sh](scripts/bootstrap-handoff.sh) — optional add-on that installs the two files above's own plumbing (the git `post-commit` hook and the `.claude/settings.json` `PostToolUse` hook) automatically, the first time a session starts in a repo that has this `handoff/` folder and a `.git` — so a brand-new project gets the automatic pipeline with zero manual per-repo setup. Wired in once, globally, via `~/.claude/settings.json`'s `SessionStart` hook — see the install block in this script's own header comment. Without this, `log-progress.sh` and `trigger-handoff-update.sh` still work, but each new repo needs their install steps done by hand once (see README.md's "Setup").
